/* ═══════════════════════════════════════════════
   magmacrunch media — collective works template
   templates/collective_works.js

   Multi-artist variant of artist_works.js.
   Fetches works from all IDs in COLLECTIVE_CONFIG.ids,
   ...
   Requires window.COLLECTIVE_CONFIG (defined in stub) = {
     ...
     lyricsPath: string  — path to lyrics directory (optional)
     lyricsMap:  object  — title → filename mapping (optional)

     // Theme overrides (optional):
     cssVarPrefix:  string — CSS var prefix (e.g. 'thld' for var(--thld-yellow))
...
(function () {
    ...
    style.textContent = `
        ...
        .alternate-names { font-family: 'Courier Prime', monospace; font-size: 11px; color: ${mutedVar}; font-style: italic; }
    ...
    async function fetchWorkRelsForArtist(artistId) {
        const data = await fetchWithRetry(`https://musicbrainz.org/ws/2/artist/${artistId}?inc=work-rels&fmt=json`);
    ...
    (async () => {
        ...
        try {
            let workRels;
            if (_cache?.works?.artistWorkRels?.relations) {
                workRels = _cache.works.artistWorkRels.relations;
                statusEl.textContent = `loaded ${workRels.length} works from cache`;
            } else {
                statusEl.textContent = 'fetching works for all members…';
                const perArtist = await Promise.all(C.ids.map(id => fetchWorkRelsForArtist(id)));
                const seen = new Set();
                workRels = [];
                for (const batch of perArtist) {
                    for (const rel of batch) {
                        const wid = rel.work?.id;
                        if (wid && !seen.has(wid)) { seen.add(wid); workRels.push(rel); }
                    }
                }
            }
            ...
            for (let i = 0; i < workRels.length; i++) {
                ...
                try {
                    const w = await cached(`work/${workId}?inc=artist-rels+label-rels+url-rels+place-rels+tags+work-rels+aliases+recording-rels&fmt=json`, _cache?.works?.details?.[workId]);
                    ...
                    for (const r of recRels) {
                        ...
                        if (!recId) continue;
                        try {
                            const cachedFlags = _cache?.works?.recordingFlags?.[recId];
                            if (cachedFlags) {
                    ...
                    const recordings = recsWithDetails.map(({ rel: r, isVideo, disambiguation }) => {
                        ...
                        if (disambiguation) title += ` <span style="color:${mutedVar};font-style:italic;">(${esc(disambiguation)})</span>`;
                    ...
                    // other rels (urls, other works, etc.)
            ...
            console.error('collective_works.js fetch failed:', err);
        }
