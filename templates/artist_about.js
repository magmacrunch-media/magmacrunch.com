/* ═══════════════════════════════════════════════
   magmacrunch media — about page template
   templates/artist_about.js
...
(function () {
    ...
    if (!C) { console.error('artist_about.js: window.ARTIST_CONFIG is not defined'); return; }
    ...
    style.textContent = `
        ...
        .bio-section { width: 100%; max-width: 960px; animation: fadeUp 0.5s 0.1s ease both; }
        .bio-section p { font-family: 'Courier Prime', monospace; font-size: 14px; color: var(--white); line-height: 1.8; margin-bottom: 16px; opacity: 0.9; }
        .bio-section .link-row { margin-top: 20px; display: flex; gap: 12px; flex-wrap: wrap; }
        .bio-section .ext-link { font-family: 'Press Start 2P', monospace; font-size: 6px; color: ${accentVar}; text-decoration: none; letter-spacing: 0.1em; padding: 7px 12px; border: 1px solid ${accentVar}; transition: background 0.15s, color 0.15s; }
        .bio-section .ext-link:hover { background: ${accentVar}; color: var(--black); }
    `;
    ...
    if (subNavEl) {
        subNavEl.innerHTML = [
            ...
            ...(C.siblings || []).filter(s => s !== 'about').map(s => {
